...
router.get('/offer', requireAuth, (_req, res) => {
  ...
  const rows = db.prepare(`
    SELECT * FROM groups WHERE asignatura IS NOT NULL ORDER BY semestre_num, asignatura
...
router.post('/', requireAuth, requireAdmin, (req, res) => {
  const {
    ...
    asignatura, horas, programaId, semestreNum, grupoSeccion, jornada,
  } = req.body as {
    ...
    asignatura?: string; horas?: number; programaId?: string;
    semestreNum?: number; grupoSeccion?: string; jornada?: string
  }
  ...
  try {
    db.prepare(`
      INSERT INTO groups
        (id, codigo, nombre, semestre, cupo_max, cupo_planeado, teacher_id, docente,
         docente_iniciales, estado_prog, asignatura, horas, programa_id, semestre_num,
         grupo_seccion, jornada)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
...
router.put('/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  const {
    ...
    teacherId, docente, docenteIniciales, estadoProg,
    cupoMax, cupoPlaneado, asignatura, horas,
    programaId, semestreNum, grupoSeccion, jornada, alerta,
  ...
  if (jornada !== undefined) { fields.push('jornada = ?'); vals.push(jornada as SQLInputValue) }
  ...
  res.json(mapGroup(db.prepare('SELECT * FROM groups WHERE id = ?').get(id) as Record<string, unknown>))
})
